...
describe "BBCodePreviewView", ->
  ...
  beforeEach ->
    filePath = atom.project.getDirectories()[0].resolve('subdir/file.txt')
  ...
  describe "::constructor", ->
    it "shows a loading spinner and renders the bbcode", ->
      preview.showLoading()
      expect(preview.element.querySelector('.bbcode-spinner')).toBeDefined()
  ...
  describe "serialization", ->
    ...
    it "does not recreate a preview when the file no longer exists", ->
      filePath = path.join(temp.mkdirSync('bbcode-preview-'), 'foo.md')
    ...
    it "serializes the editor id when opened for an editor", ->
      ...
      runs ->
        preview = new BBCodePreviewView({editorId: atom.workspace.getActiveTextEditor().id})
  ...
  describe "when core:save-as is triggered", ->
    ...
    it "saves the rendered HTML and opens it", ->
      ...
      bbcodePreviewStyles = [
        {
          rules: [
            createRule ".bbcode-preview", "{ color: orange; }"
          ]
        }, {
          rules: [
            createRule ".not-included", "{ color: green; }"
            createRule ".bbcode-preview :host", "{ color: purple; }"
          ]
        }
      ]
      ...
      atomTextEditorStyles = [
        ...
        "atom-text-editor .hr { background: url(atom://bbcode-preview/assets/hr.png); }"
      ]
      ...
      runs ->
        ...
        spyOn(preview, 'getDocumentStyleSheets').andReturn(bbcodePreviewStyles)
